...
const init_phones = ["HD560S"],                      // Optional. Which graphs to display on initial load. Note: Share URLs will override this set
      DIR = "data_oe/",                                // Directory where graph files are stored
      ...
      watermark_text = "https://hitcoder9768.github.io/HitFi",                 // Optional. Watermark appears behind graphs
...
function initTsvParse() {
    ...
    tsvParse = fr => d3.csvParseRows(fr).slice(dataStart,dataEnd)
        .map(r=>r.map(d=>+d));
}
initTsvParse();
